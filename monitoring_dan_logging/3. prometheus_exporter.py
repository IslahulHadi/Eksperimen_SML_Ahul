...
Prometheus Exporter untuk Model Customer Segmentation
...
ERROR_COUNT = Counter('http_requests_error_total', 'Total number of failed requests')
SUCCESS_COUNT = Counter('http_requests_success_total', 'Total number of successful requests')
REQUEST_SIZE = Histogram('http_request_size_bytes', 'Size of HTTP request payload (bytes)')
RESPONSE_SIZE = Histogram('http_response_size_bytes', 'Size of HTTP response payload (bytes)')
...
def predict():
    """Endpoint untuk prediksi — forward ke MLflow serving."""
...
if __name__ == '__main__':
    print("=" * 50)
    print("Customer Segmentation — Prometheus Exporter")
